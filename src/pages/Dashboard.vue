            ...
            <v-row class="mx-auto" style="max-width: 1000px">
                <v-col>
                    <v-card v-if="botStatus" color="rgba(0, 0, 0, 0)" flat>
                        ...
                        <v-card-text>
                            MtheBot_ is currently sitting in your chat awaiting commands! You may disable MtheBot_ and
                            it will leave your chat.
                        </v-card-text>
                        <v-card-actions>
                    ...
                    <v-card v-else color="rgba(0, 0, 0, 0)" flat>
                        ...
                        <v-card-text
                            >MtheBot_ must be enabled in order to enhance your chat experience. Enable MtheBot_ so it
                            can join your chat. This action will set MtheBot_ as a moderator.</v-card-text
                        >
                        <v-card-actions>
            ...
            <v-row class="mx-auto" style="max-width: 1000px">
                ...
                <v-col>
                    <v-card flat color="rgba(0, 0, 0, 0)" height="100%" class="d-flex flex-column">
                        ...
                        <v-card-text
                            >Consider supporting the development and maintenance of MtheBot_ by supporting me on GitHub
                            Sponsors or donating through PayPal!</v-card-text
                        >
                        <v-card-actions class="mt-auto">
                            <v-btn
                                text
                            ...
                            <v-btn
                                text
                                color="primary"
                                href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=9WS3KJPAV8JDJ&currency_code=USD"
                                target="_blank"
                                rel="noopener noreferrer"
                            >
                                Donate
...
export default {
    ...
    data: function() {
        return {
            ...
            navCardData: [
                {
                    title: "Commands",
                    content: "Modify MtheBot_'s commands for your channel",
                    route: "/commands",
                    action: "Go to Commands",
                },
                {
                    title: "Events",
                    content: "Modify MtheBot_'s channel event messages",
                    route: "/events",
                    action: "Go to Events",
                },
                {
                    title: "Timed Messages",
                    content: "Modify MtheBot_'s timed channel messages",
                    route: "/timers",
                    action: "Go to Timers",
                },
            ],
    ...
    mounted() {
        ...
        this.axios
            .get(`/chats/${channel}`, { headers: { Authorization: `Bearer ${this.$auth.accessToken}` } })
            .then((res) => {
    ...
    methods: {
        disableBot: function() {
            const channel = this.$store.state.userData.id;
            this.axios
                .delete(`/chats/${channel}`, { headers: { Authorization: `Bearer ${this.$auth.accessToken}` } })
                .then((res) => {
                    if (res.status === 200) {
                        this.botStatus = false;
                    }
        ...
        enableBot: function() {
            const channel = this.$store.state.userData.id;
            this.axios
                .post(`/chats/${channel}`, {}, { headers: { Authorization: `Bearer ${this.$auth.accessToken}` } })
                .then((res) => {
                    if (res.status === 200) {
                        this.botStatus = true;
                    }
